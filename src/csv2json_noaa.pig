...
myOutput = FOREACH myInput GENERATE STATION_NAME AS id,
CONCAT(SUBSTRING(date, 0, 4), CONCAT('-',CONCAT(SUBSTRING(date, 4, 6),CONCAT('-',SUBSTRING(date, 6, 8))))) AS date, lon, lat, ELEVATION AS z;
